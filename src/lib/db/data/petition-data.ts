...
export async function fetchPetitions() {
    noStore()
    try {
        const data = await prisma.petition.findMany({where: {showDetails: true}, orderBy: {createdAt: 'desc'}, take: 9})
        return await Promise.all(data.map(async (petition) => {
            const user = await clerkClient.users.getUser(petition.userId);
            return {
                ...petition,
                createdAt: formatDateToLocal(petition.createdAt),
...
export async function fetchPetitionStatistics() {
    try {
        ...
        return query.map(item => {
            return {
                ...item,
                number: `${(item._count.district).toLocaleString()} petition signature${item._count.district === 1 ? '' : 's'}`
            }
        })
    } catch (error) {
