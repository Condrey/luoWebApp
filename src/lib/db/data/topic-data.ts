...
export async function fetchTopics() {
    ...
    try {
        ...
        return await Promise.all(data.map(async (item) => {
            ...
            const numberOfLikes = item.topicLikeOrDisLikes.filter(value => value.likeOrDislike === 'LIKE').length || 0
            const numberOfDisLikes = item.topicLikeOrDisLikes.filter(value => value.likeOrDislike === 'DISLIKE').length || 0
            const wasUpdated = item.updatedAt > item.createdAt
            const createdUpdatedAtTimestamp = wasUpdated ? item.updatedAt : item.createdAt

            return {
                ...item,
                createdUpdatedAtTimestamp: `${formatDateToLocal(createdUpdatedAtTimestamp)} ${wasUpdated ? ' (edited)' : ''} `,
                userName: user.firstName || user.emailAddresses[0].emailAddress,
                imageUrl: user.imageUrl,
                numberOfComments: `${numberOfComments} ${numberOfComments === 1 ? 'reply' : 'replies'}`,
...
export async function fetchUniqueTopic(id: string) {
    ...
    try {
        ...
        const numberOfLikes = data.topicLikeOrDisLikes.filter(value => value.likeOrDislike === 'LIKE').length || 0
        const numberOfDisLikes = data.topicLikeOrDisLikes.filter(value => value.likeOrDislike === 'DISLIKE').length || 0
        const wasUpdated = data.updatedAt > data.createdAt
        const createdUpdatedAtTimestamp = wasUpdated ? data.updatedAt : data.createdAt
        const wholeComments = await Promise.all(data.comments.map((async item => {
            ...
            return {
                ...item,
                createdUpdatedAtTimestamp: `${formatDateToLocal(createdUpdatedAtTimestamp)} ${wasUpdated ? ' (edited)' : ''} `,
                userName: commentUser.firstName || commentUser.emailAddresses[0].emailAddress,
