        ...
        <div className='flex flex-col gap-4'>
            {
                discussions.map((discussion) => {
                    return <Link href={`/grievances/chat/${discussion.id}`} key={discussion.id}
                        ...
                        <div className='flex gap-4'>
                            ...
                            <div className='flex flex-col  dark:text-slate-500'>
                                ...
                                <span className='text-xs'>-{discussion.createdUpdatedAtTimestamp}</span>
                            </div>
                        </div>
                        ...
                        <DialogFooter className='flex flex-wrap items-center justify-between gap-3'>
                            <div className='flex gap-2 items-center grow justify-center'>
                                ...
                                <div
                                    className={cn(buttonVariants({variant: 'ghost'}), `flex items-end gap-1 cursor-pointer`)}>
                                    <LucideThumbsDown/><span>{discussion.numberOfDisLikes}</span>
                                </div>
                                <span
                                    className={cn(' flex items-end gap-1 text-slate-700')}>
                                                            <LucideReply/><span>{discussion.numberOfComments}</span>

                            </span>
                            </div>
                            <Link href={`/grievances/chat/${discussion.id}`}
                                  className={buttonVariants({variant: 'destructive'})}>Join</Link>
                            <div className={cn(discussion.authorId !== userId && 'hidden')}>
                                <EditTopicButton topicToEditJsonString={JSON.stringify(discussion)}/>
                            </div>
                        </DialogFooter>
...
export async function RecentDiscussionsSkeleton() {


    return <div className='flex flex-col gap-3 flex-grow   h-full rounded-md p-3 border'>
        <span className='text-xl md:text-2xl   flex justify-center'>Recent Discussions</span>
        <div className='flex flex-col gap-4'>
            {Array.from({length: 3}, (_, index) => {
                return <div key={index}
                            className='flex flex-col drop-shadow-md gap-2 border rounded-md p-3  w-full cursor-pointer'>
                    <div className='flex gap-4'>
