...
export default function PlayListContainer({
  ...
  const noOfVideos =
    videoNumber === 1 ? "1 video" : `${videoNumber.toLocaleString()} videos`;
  return (
    <div
    ...
    >
      <Link href={`/grievances/video-gallery/playlist/${playlist.id}`}>
        <div className="relative bg-amber-400 dark:bg-amber-300 text-slate-950 h-full">
          <div className="flex flex-col gap-2 border-2 shadow-2xl hover:drop-shadow-xl h-full">
            ...
            <div className="text-xl font-bold px-4 pb-2 align-baseline">
              ...
              {`${playlist.name}`}
              <br />
              <span className=" text-sm">{noOfVideos} </span>
            </div>
          </div>
          {isLoading && (
