import {
  fetchPetitionNumber,
  fetchPetitions,
} from "@/lib/db/data/petition-data";
...
export default async function DominantFiguresPetitions() {
  ...
  const otherPetitions: number | null =
    petitions.length === 9
      ? Math.ceil(numberOfPetitions - petitions.length)
      : null;
  return (
    <div className="flex flex-col gap-3">
      <span className="text-2xl font-bold">Also signed By:</span>
      <span className="">
        NB: Only people who signed the petition with{" "}
        <strong className=" inline-flex gap-1 items-center justify-center before:content-check-box  ">
          Show my details
        </strong>{" "}
        , will appear here.
      </span>
      <div className=" grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
        {petitions.map((petition) => (
          <div
            key={petition.id}
            className="flex flex-col gap-2 rounded-md p-3 items-center  bg-blue-200/10  dark:bg-accent/20  border border-b-blue-200 dark:border-accent-foreground/50"
          >
